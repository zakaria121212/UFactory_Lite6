...
# ...existing code...
    ...
    # Add gripper controller also for 'lite'
    if add_gripper.perform(context) in ('True', 'true'):
        if robot_type.perform(context) == 'lite':
            # Pick ONE name to match your YAML:
            controllers.append('{}lite_gripper_traj_controller'.format(prefix.perform(context)))
            # If your YAML uses 'lite_gripper_trajectory_controller', use this instead:
            # controllers.append('{}lite_gripper_trajectory_controller'.format(prefix.perform(context)))
        else:
            controllers.append(
                '{}{}_gripper_traj_controller'.format(prefix.perform(context), robot_type.perform(context))
            )

    if add_bio_gripper.perform(context) in ('True', 'true') and robot_type.perform(context) != 'lite':
