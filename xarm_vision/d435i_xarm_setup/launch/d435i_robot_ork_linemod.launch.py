...
def launch_setup(context, *args, **kwargs):
    ...
    ork_linemod_detection = Node(
        package='object_recognition_core',
        executable='detection',
        arguments=['-c $(find object_recognition_linemod)/conf/detection.ros.ork']
    )

    tf_object_to_base = Node(
        package='d435i_xarm_setup',
        executable='tf_object_to_base',
    )
