...
def launch_setup(context, *args, **kwargs):
    ...
    # Launch a gripper planner node when requested, including for lite
    if add_gripper.perform(context) in ('True', 'true') and use_gripper_node.perform(context) in ('True', 'true'):
        if robot_type.perform(context) == 'lite':
            planning_group = 'lite_gripper'
        elif robot_type.perform(context) == 'uf850':
            planning_group = 'uf850_gripper'
        else:
